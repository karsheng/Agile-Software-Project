...
def TwitLoader(product_name, startDate, endDate, daily_limit):
    limit = daily_limit
    ...
    for myDay in datesOnly:
        ...
        myDF = pd.DataFrame()
        myDate = str(myDay)
        fromDay = str(myDay - timedelta(days=1))
        query = "{0} lang:en since:{1} until:{2}".format(product_name, fromDay, myDate)
